...
## 2.1 Post-collection data quality assessment
...
### 2.1.2 Missing values and outlier detection
...
**Z-Score Method** The z-score measures the distance of a value from the mean in units of standard deviation. Values beyond ±3 standard deviations are commonly treated as potential outliers. Under a normal distribution, such values occur in approximately 0.3 per cent of cases. This method is effective when the distribution is roughly symmetric and free of extreme values.

**Modified Z-Score Method** Because means and standard deviations can be distorted by outliers, a more robust alternative uses the median and the Median Absolute Deviation (MAD). A value is typically classified as an outlier if its modified z-score exceeds ±3.5. A scaling constant (0.6745) is applied to ensure comparability with the standard z-score under normality.

**Box-Plot Rule** This rule is based on the interquartile range (IQR), defined as the difference between the third quartile (Q3) and the first quartile (Q1). Observations falling below *Q1 ? 1.5 × IQR* or above *Q3 + 1.5 × IQR* are considered potential outliers. A stricter threshold uses 3 × IQR. This method is robust to extreme values and does not require the assumption of normality.

Several of the statistical methods described above assume an approximately normal distribution. Many survey variables, particularly income or welfare indicators, are often skewed. In such cases, applying a transformation before outlier detection may improve performance. The natural logarithm is a simple and commonly used transformation to reduce skewness[[1]](#footnote-1).
...
Different data entry programs may export the dataset in their own specific structure. For example, Survey Solutions[[2]](#footnote-2) exports one dataset per reporting level (i.e. one dataset for all the individual level data from different sections, one for household level, etc.).
...
The need for data anonymization stems from its role in providing ethical and legal safeguards, including the protection of respondent privacy and confidentiality, adherence to commitments made during informed consent, and compliance with national data-protection laws and international guidelines such as the UN Fundamental Principles of Official Statistics[[4]](#footnote-4)and the World Bank Data Quality Policy. By minimizing disclosure risks, especially when microdata is shared with external users, anonymization also enables safe data dissemination, fostering greater research collaboration without compromising participant trust.
...
## 2.5 Reproducible Workflows and Version Control
...
## 2.6 Secure Handling of Raw and Intermediate Data
...
## 2.7 Communicating and Escalating Data Quality Issues
...
## 2.8 Coherence, Comparability, and International Classifications
...
## 2.9 Metadata Standards and Deposit
...
## 2.10 Access, Dissemination, and Archiving
...
## 2.11 Interoperability and Responsible Use of Automated Tools
